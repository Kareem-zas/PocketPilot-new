...
class _SharedGoalsPageState extends State<SharedGoalsPage> {
  ...
  void initState() {
    ...
    SocketService.init();
    SocketService.onGoalUpdated((data) {
      // Fetch updated goals when notified
      // debugPrint('WebSocket Event: goalUpdated $data');
  ...
  Future<void> _loadSharedGoals({required bool showLoading}) async {
    ...
    try {
      ...
      // Join the WebSocket room for each shared goal to receive live updates
      for (var goal in goals) {
        if (goal['_id'] != null) {
          SocketService.joinGoalRoom(goal['_id']);
        }
      }
    } catch (e) {
      if (!mounted) return;
