...
class _FixedExpensePageState extends State<FixedExpensePage> {
  ...
  Future<void> _initData() async {
    _currencySymbol = await CurrencyService.getSymbol();
    _conversionRate = await CurrencyService.getConversionRate();
    await _loadExpenses();
  }
  ...
  Future<void> _fetchTip(List<dynamic> items, double total) async {
    setState(() => _loadingTip = true);
    final tip = await AiInsightsService.fetchFixedExpenseTip(
      activeItems: items,
      monthlyTotal: total * _conversionRate,
            ...
            children: [
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      "$_currencySymbol${(monthlyTotal * _conversionRate).toStringAsFixed(2)}",
                      style: TextStyle(
                        fontSize: 32,
              ...
              Container(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text("Pilot Tip", style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    if (_loadingTip)
                      const Row(
                        children: [
                          ...
                          SizedBox(width: 8),
                          Text("AI Pilot is analyzing your subscriptions...", style: TextStyle(color: Colors.white70, fontSize: 12)),
                        ],
                      )
                    else
                      Text(
                        _pilotTip ?? "Review your active subscriptions to save money.",
